$(document).ready(function () {
				...
				if (id.match(/ausgabe[0-9]/)) {
					...
					if (value.parent_id == parent_id) {
						if (value.name.match(reg)) {
							...
							html_code += '<option value="' + value.name.replace(/(GW[0-9]+)[PB][0-9]+(n[0-9]+)/g, "$1$2") + '" id="' + value.id + '">Kapitel ' + value.name.replace(/GW[0-9]+[PB]([0-9]+)n[0-9]+/g, "$1") + '</option>';
						} else {
	...
	$(document).on('change', '#ausgabe2', function () {	
		...
		if (ausgabe_id != '') {
			...
			$('#ansicht2').html('<option value="">Ansicht wählen</option>');
		}
	});
	...
	$(document).on('change', '#kapitel2', function () {		
		...
		if (kapitel_id != '') {
			load_json_data('ansicht2', kapitel_id);
		} else {
			$('#ansicht2').html('<option value="">Ansicht wählen</option>');
		}
	});
	
	$(document).on('change', '#ansicht2', function () {
		var vers = $(this).find('option:selected').text();
		localStorage.setItem('vers2', vers);
	});
});
...
$(document).ready(function () {
	...
	$('#ansicht2').trigger("chosen:updated");
});